...
if __name__ == "__main__":
    ...
    join.coalesce(1).saveAsTextFile("out/age_address_join.text")

    leftOuterJoin = ages.leftOuterJoin(addresses)
    leftOuterJoin.coalesce(1).saveAsTextFile("out/age_address_left_out_join.text")

    rightOuterJoin = ages.rightOuterJoin(addresses)
    rightOuterJoin.coalesce(1).saveAsTextFile("out/age_address_right_out_join.text")
